...
function canonicalWfmLabel(raw) {
  return String(raw || '')
    .normalize('NFC')
    .replace(/\s*\([^)]*\)\s*/g, ' ')
    .replace(/\s+/g, ' ')
...
function isoWeekday(isoDay) {
  const [y, m, d] = isoDay.split('-').map(Number);
  const wd = new Date(Date.UTC(y, m - 1, d)).getUTCDay();
  return wd === 0 ? 7 : wd;
}

function intersectingSlots(startMin, endMin) {
  const slots = [];
...
async function loadImportSettings(client) {
...
function aggregateDetails(details, weekdays, skipHolidays) {
  const sets = new Map();
